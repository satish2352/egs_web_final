...
@section('content')
    ...
    <div class="main-panel">
            ...
            <div class="page-header">
                <h3 class="page-title">
                    ...
                    @if (in_array('per_add', $data_permission))
                        <a href="{{ route('add-relation') }}" class="btn btn-sm btn-primary ml-3">+
                            Add</a>
                    @endif
                                                        ...
                                                        <td>
                                                            <div class="d-flex">
                                                                @if (in_array('per_update', $data_permission))
                                                                    <a
                                                                        href="{{ route('edit-relation', base64_encode($item->id)) }}"
        ...
        <form method="POST" action="{{ url('/delete-relation') }}" id="deleteform">
            @csrf
            <input type="hidden" name="delete_id" id="delete_id" value="">
        </form>
        <form method="POST" action="{{ url('/show-relation') }}" id="showform">
            @csrf
            <input type="hidden" name="show_id" id="show_id" value="">
        </form>
        <form method="POST" action="{{ url('/update-one-relation') }}" id="activeform">
            @csrf
