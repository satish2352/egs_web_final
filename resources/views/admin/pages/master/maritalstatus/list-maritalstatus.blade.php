...
@section('content')
    ...
    <div class="main-panel">
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="table-responsive">
                                        <table id="order-listing" class="table table-bordered">
                                            ...
                                            <tbody>
                                                @foreach ($maritalstatus_data as $item)
                                                    <tr>
                                                        <td>{{ $loop->iteration }}</td>
                                                        <td>{{ strip_tags($item->maritalstatus) }}</td>
                                                        ...
                                                        <td>
                                                            <div class="d-flex">
                                                                @if (in_array('per_update', $data_permission))
                                                                    <a
                                                                        href="{{ route('edit-maritalstatus', base64_encode($item->id)) }}"
        ...
        <form method="POST" action="{{ url('/delete-maritalstatus') }}" id="deleteform">
            @csrf
            <input type="hidden" name="delete_id" id="delete_id" value="">
        </form>
        <form method="POST" action="{{ url('/show-maritalstatus') }}" id="showform">
            @csrf
            <input type="hidden" name="show_id" id="show_id" value="">
        </form>
        <form method="POST" action="{{ url('/update-one-maritalstatus') }}" id="activeform">
            @csrf
